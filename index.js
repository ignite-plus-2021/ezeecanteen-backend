...
app.post("/orderstatus1", (req, res) => {

    const orderid = req.body.Oid;
    const ordNo = req.body.oNo;
    console.log(orderid, ordNo)
    db1.query("select orderStatus from orders where id=? and orderNo=?", [orderid, ordNo], (err, result) => {
        res.send(result);
    });
...
app.post("/api/forgot", (req1, res1) => {
    ...
    bcrypt.hash(password, 10, function (err, hash) {
        console.log(hash)
        if (email == "" || password == "") {
            res1.send({ message: "Enter Valid Email id and Valid Password to Reset Password !!!" })
        }
        else {
